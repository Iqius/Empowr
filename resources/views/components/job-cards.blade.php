@foreach ($jobs as $job)
@php
$deadlinePromotion = \Carbon\Carbon::parse($job->deadline_promotion);
@endphp

@if($job->status == 'open' && $deadlinePromotion->gte(now()))
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-xs text-gray-400">Penutupan <span
                    class="font-semibold text-gray-500">{{ $deadlinePromotion->translatedFormat('d F Y') }}
                </span></p>
        </div>
